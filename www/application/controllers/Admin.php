...
class Admin extends CI_Controller {
	...
	public function createAccount() {
        ...
        if (!empty($_POST)){
            ...
            if (!empty($errors)) {
                ...
                if($this->user->createAccount($account["username"],$password,$phoneNum,$userType,$fName,$lName)){
                    echo "DB success";
                }
                else {
                    echo "DB failure";
    ...
    public function createMassAccounts($numAccounts){
        ...
        for ($i = 0; $i< $numAccounts; $i++){
            $account["firstName"] = $names[rand(0,sizeof($names)-1)];
            $account["middleName"] = $names[rand(0,sizeof($names)-1)];
